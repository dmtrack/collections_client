...
const EditCollection = () => {
    ...
    const { collection, collectionConfig } = useCollection(
        Number(collectionId)
    );
    ...
    const handleSubmit = (data: ICollectionFormValues) => {
        const itemConfigs = data.itemConfigs.filter(
            (config) => config.type && config.label
        );
        if (data.image && data.image?.size > MAX_IMAGE_SIZE)
        ...
        dispatch(
            editCollection(editedCollection, itemConfigs, navigate, data.image)
        );
    };
    if (!collection) {
        navigate(-1);
